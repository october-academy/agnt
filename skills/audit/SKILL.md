...
- 1차: 상태/강도/유형을 3-5개 선택지로 받는다
- 2차: 꼭 필요한 경우에만 한 줄 근거를 받는다
- URL, 숫자, 실제 관찰 메모, 제품 한 줄 설명만 자유 입력 허용
- 선택지는 하드코딩하지 않고 `project.*`, stage, 직전 답변을 바탕으로 동적으로 만든다
...
각 질문은 먼저 선택지로 받고, 선택지만으로 부족할 때만 한 줄 근거를 받는다.

**Q1 Demand Reality**
AskUserQuestion: "네 제품이 내일 사라지면 진짜 화날 사람이 있어?"
- 강도 기반 선택지 3-4개를 동적으로 생성
- 예: 없음 / 애매함 / 1-2명 명확 / 여러 명 명확

**Q2 Status Quo**
AskUserQuestion: "그 사람들은 지금 이 문제를 어떻게 버티고 있어?"
- 현재 대안 유형을 3-5개 동적으로 생성
- `project.problem`과 ICP 맥락에 안 맞는 옵션은 넣지 않는다

**Q3 Desperate Specificity**
AskUserQuestion: "ICP는 어디까지 좁혀져 있어?"
- buyer clarity 수준에 맞는 선택지 3-4개를 동적으로 생성

**Q4 Narrowest Wedge**
AskUserQuestion: "이번 주 안에 돈 받을 수 있는 가장 작은 버전은?"
- 현재 제품 단계에 맞는 wedge 상태 선택지 3-4개를 동적으로 생성

**Q5 Observation & Surprise**
AskUserQuestion: "실제 사용/행동을 본 적이 있어?"
- evidence 강도 선택지 3-4개를 동적으로 생성

**Q6 Future-Fit**
AskUserQuestion: "3년 뒤 이 문제는 어떻게 될 것 같아?"
- 미래 중요도 선택지 3-4개를 동적으로 생성
...
**Buyer Clarity가 낮으면**:
AskUserQuestion: "ICP 명확도는 어느 쪽이야?"
- buyer clarity를 설명하는 선택지 3-4개를 동적으로 생성

**Problem Clarity가 낮으면**:
AskUserQuestion: "현재 대안의 불만은 어느 쪽이 더 커?"
- 현재 문제 맥락에서 plausible한 불만 선택지 3-5개를 동적으로 생성

**Offer Clarity가 낮으면**:
AskUserQuestion: "우리 오퍼 상태는 어디에 가까워?"
- 오퍼 선명도 상태 선택지 3-4개를 동적으로 생성

**Evidence Clarity가 낮으면**:
AskUserQuestion: "지금까지 실제로 해본 검증은?"
- 현재 프로젝트 단계에서 가능한 검증 유형 3-5개를 동적으로 생성

**추가 질문 (순차, 한 번에 하나)**:

AskUserQuestion: "기술 스택은 어디에 가까워?"
- 현재 제품 형태에 맞는 기술/전달 방식 후보 3-5개를 동적으로 생성
- 마지막 선택지는 항상 `기타 — 한 줄 입력`
...
Track A: "오늘 /agnt:icp로 ICP 문서를 확정해. 그 다음 /agnt:landing으로 넘어가."
Track B: "오늘 /agnt:icp를 실행해. 인터뷰할 사람 기준부터 문서로 고정해."
Track C: "오늘 /agnt:discover를 실행해. 문제를 고른 뒤 /agnt:icp로 좁혀."
