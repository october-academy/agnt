...
- interview/journey-brief에 나온 현재 대안이 있으면 그 후보를 먼저 3-5개 선택지로 제시
- 없으면 문제/ICP 기준으로 plausible한 대안 후보를 동적으로 생성
- 마지막 선택지만 `기타 — 직접 입력`
...
- 각 대안별로 ICP 기준 약점 후보 3-5개를 동적으로 생성
- 마지막 선택지만 `기타 — 한 줄 입력`
...
- 차별화 포인트 후보 3-5개를 경쟁 분석 결과 기반으로 동적으로 생성
- 마지막 선택지만 `직접 한 줄 작성`
