...
Week 1 중반 데이터를 3축으로 종합합니다:

| 축 | 질문 | 데이터 소스 |
|---|---|---|
| Customer | 타겟이 이 문제를 인정하는가? | 대화 기록, 피드백 |
| Channel | 타겟에게 도달할 수 있는가? | `get_links` 클릭 데이터 |
| Monetization | 돈을 낼 의향이 있는가? | 가격 질문 응답 |

각 축에 🟢(확인됨) / 🟡(약한 신호) / 🔴(미확인) 표시.

이 스코어카드가 SPEC v2의 입력이 됩니다.
...
{{character.runway}}개월이면
3개월 시행착오를 감당할 수 있어?

{미존재 시}
3개월 시행착오를 감당할 여유가 있어?
