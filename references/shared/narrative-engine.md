...
- `on_complete`: 블록 완료 시 추가 동작 (`save_character`, `save_interview` 등)
  - `save_character`는 MCP 도구명이 아니라 로컬 state의 `character`를 갱신하는 블록 액션입니다.
...
### 이전 Day 확정 데이터 기반 압축 규칙

이전 Day에서 확정된 데이터(character.exploration, builderContext, interview 등)가
현재 블록의 내용과 중복될 때 아래 규칙으로 압축합니다.

이 규칙은 **동일한 정보를 재수집하는 중복**에만 적용합니다.
새 프레임워크나 개념을 가르치는 블록에는 적용하지 않습니다.

#### Teach 모드 — CONTEXT_GATE

블록에 `## CONTEXT_GATE` 섹션이 있으면:

1. 게이트 조건(state.json 필드 존재 여부)을 확인
2. 조건 충족: CONTEXT_GATE의 리캡 대사를 출력하고, 지정된 섹션을 건너뛰어 MOVE로 진행
3. 조건 미충족: CONTEXT_GATE를 무시하고 다음 섹션(SCENE 1 등)부터 정상 진행
4. CONTEXT_GATE를 건너뛴 경우에도 블록 완료 시 state.json 갱신(completedBlocks, currentBlock, lastNpc, lastAction, lastLocation)은 정상 수행

CONTEXT_GATE가 없는 블록은 기존 흐름 그대로 진행합니다.

#### 대사 수준 조건 분기 — DIALOGUE_GATE

NPC 대사 안에서 state.json 필드 존재 여부에 따라 대사를 분기할 수 있습니다.
CONTEXT_GATE가 **섹션 단위** 건너뛰기라면, DIALOGUE_GATE는 **대사 단위** 분기입니다.
...
{미존재 시}
"시간은 누구한테나 줄어들어.
오늘 하루를 검증에 썼다는 게 중요해."
```

규칙:

1. `{state.XXX 존재 시}` / `{미존재 시}` 쌍으로 사용
2. 조건은 state.json 필드 존재 여부만 판단 (값 비교 불가)
3. `{미존재 시}` 블록은 필수 — fallback 대사가 반드시 있어야 함
4. 한 NPC 대사 블록 안에 최대 1개의 DIALOGUE_GATE만 사용
5. `{{variable|fallback}}` 보간과 조합 가능

CONTEXT_GATE와의 차이:

| | CONTEXT_GATE | DIALOGUE_GATE |
|---|---|---|
| 범위 | 섹션 전체 | NPC 대사 1-2줄 |
| 위치 | `## CONTEXT_GATE` 헤더 | NPC 대사 블록 내부 |
| 효과 | 섹션 건너뛰기 | 대사 분기 |
| fallback | 정상 진행 | `{미존재 시}` 대사 |

#### Talk 모드 — 체크리스트 2-tier

완성 체크리스트에 "확인 항목"과 "신규 발견 항목"이 구분되어 있으면:

- **확인 항목**: state.json에 해당 데이터가 있으면 NPC가 데이터를 인용하며 "맞아?" 확인 1턴. 유저가 동의하면 체크, 부정하면 재탐색
- **신규 발견 항목**: 처음부터 탐색 (기존 인터뷰 규칙 적용)
- 데이터가 없으면 모든 항목을 구분 없이 처음부터 탐색

2-tier 구분이 없는 체크리스트는 기존 규칙 그대로 진행합니다.
...
블록에서 전문 용어가 등장할 때 아래 4가지 카테고리에 따라 처리합니다.

#### 카테고리 1: 세계관 어휘

Agentic30 고유 개념. NPC가 **장면 → 패턴 → 이름** 순서로 첫 등장 시 가르치고, 이후 자유 사용.

| 용어 | 첫 도입 위치 | 도입 NPC |
|------|-------------|---------|
| 검증 | Day 0 Block 0 | 두리 |
| SPEC | Day 0 Block 0 (예고) → Day 1 Block 2 (본격) | 소리 |
| 검증 목표 | Day 0 Block 0 | 두리 |
| 검증 채널 | Day 0 Block 0 | 두리 |
| 검증 루프 | Day 0 Block 3 | 두리 |
| 수정 사항 | Day 2 Block 2 | 바리 |
| 변화량 | Day 4 Block 2 (예고: 나래) → Day 5 Block 0 (본격: 한이) | 나래 → 한이 |
| 최종 판단 | Day 6 Block 2 | 은이 |
| 고객 검증 | Day 2 Block 0 | 바리 |
| 채널 검증 | Day 2 Block 0 | 바리 |

도입 예시:
```
두리: "1인 개발자들이 '시장 반응'이라고 부르는 것 — 고객이 실제로 한 행동, 클릭, 결제 의향 — 여기선 '검증'이라 부르자."
```

#### 카테고리 2: 습관적 영어 → 한국어 교체
...
NPC가 쉬운 설명 후 영어 이름 붙여줌.

| 용어 | 도입 위치 | 도입 NPC | 설명 방식 |
|------|----------|---------|----------|
| CTA | Day 1 Block 0 | 소리 | "방문자한테 '이거 해봐'라고 말하는 버튼 — 업계에선 CTA라고 불러" |
| objection | Day 3 Block 0 | 석이 | "상대가 '그건 좀...' 하고 멈칫하는 지점 — objection이라고 해" |
| hero section | Day 1 Block 2 | 소리 | "페이지 열었을 때 제일 먼저 보이는 영역" |
| paid pilot | Day 0 Block 3 | 두리 | "돈을 내고 써보겠다는 약속" |
| KPTL | Day 7 Block 1 | 달이 | "Keep/Problem/Try/Learn" 병기 |

#### 카테고리 4: 코드 변수명 → 한국어 라벨

유저에게 절대 노출 금지. NPC 대사와 선택지에서는 한국어 라벨만 사용.

| 코드 변수명 | 한국어 라벨 |
|------------|-----------|
| `no_idea` | 아이디어 없음 |
| `idea_only` | 아이디어만 있음 |
| `landing_live` | 랜딩 운영 중 |
| `product_live` | 제품 운영 중 |
| `revenue_live` | 매출 발생 중 |
| `null` (SPEC 필드) | 아직 없음 |
| `builderContext` | 빌더 진단 결과 |
| `blank-slate` | 처음부터 시작 |

**예외**: ON_COMPLETE/ON_CONFIRM 시스템 지시문, frontmatter 필드, state.json 키의 코드명은 유지합니다. 이들은 Claude Code가 읽는 프로그래밍 지시문이므로 변환하지 않습니다.

**SPEC 템플릿**: 표시용 라벨은 이중 언어 — "핵심 가설 (Hypothesis)" 형태. 저장 키(`hypothesis`, `changes` 등)는 MCP `save_spec_iteration`이 기대하므로 변경 불가.
...
유저가 대화 중 특정 도구, CLI 명령, URL 확인 등 **실행 가능한 행동을 요청**하면 NPC 세계관보다 유저 지시가 우선합니다.

### 처리 순서

1. 요청된 도구/명령을 실행 시도 (ToolSearch, Bash, MCP 등 활용)
2. 성공 시 결과를 NPC 톤으로 대화에 반영 후 체크리스트 복귀
3. 실패 시 (도구 미설치, 접근 불가 등):
   - 실패 원인을 솔직하게 안내 + 대안 제시
   - 예: `"gws CLI가 설치돼 있지 않아. 핵심 내용을 직접 말해줄 수 있어?"`

### 금지

- 유저 요청을 NPC 캐릭터/세계관 이유로 거부 (`"그 장부는 여기서 열 수 없어"` ❌)
- 유저가 도구 사용을 요청했는데 구두 답변으로 대체 강요

### 예시

```
유저: "gws cli로 이 스프레드시트 확인해"
✅: Bash로 gws CLI 실행 시도 → 결과 반영 / 미설치 시 솔직히 안내
❌: "그 장부는 여기서 열 수 없어. 핵심만 말해줘."

유저: "posthog에서 확인해봐"
✅: PostHog MCP 쿼리 실행 → 결과를 NPC 톤으로 전달

유저: "이 URL 열어봐"
✅: WebFetch 또는 브라우저 도구로 확인 시도
```
...
- **"응, 바로 가자"**: `currentDay`의 블록 0 레퍼런스를 Read하고 바로 진행합니다 (continue.md step 4부터 재실행).
- **"아니, 오늘은 여기까지"**: NPC가 짧은 인사를 건네고 세션을 종료합니다.
...
특정 블록에서 CHECK와 MOVE 사이에 `## AUTH_GATE` 섹션이 존재할 수 있습니다.

- MCP 연결과 회원가입 확인을 순차적으로 수행하는 절차 섹션
- AUTH_GATE의 모든 조건을 충족해야 MOVE로 진행 가능
- 내부의 AskUserQuestion은 STOP 프로토콜 대상이 아님 (중간 확인용)
- ToolSearch → `get_user_info` 호출로 MCP 연결 + 프로필 완성을 검증
- 현재 Day 0 Block 0에서만 사용

### CONTEXT_GATE 특수 섹션

특정 블록에서 NPC와 첫 SCENE 사이에 `## CONTEXT_GATE` 섹션이 존재할 수 있습니다.

- 이전 Day에서 확정된 데이터가 현재 블록의 교육 내용과 중복될 때 강의를 건너뛰는 조건부 게이트
- 게이트 조건 충족 시: 리캡 대사 출력 → 지정된 섹션 skip → MOVE로 진행
- 게이트 조건 미충족 시: 게이트 무시, 다음 섹션부터 정상 진행
- 내부의 `{단일중괄호}` 변수는 AI가 state.json에서 직접 읽어 채움 (RECAP 규칙과 동일)
- CONTEXT_GATE에서 건너뛴 경우 NPC 후 페이지 브레이크(Section 7) 직후에 게이트가 발동하므로 추가 페이지 브레이크 없음
- 현재 Day 1 Block 0에서 사용
...
2. `currentBlock` 1 증가
3. 블록별 데이터 저장 (`on_complete: save_character` → 로컬 `character` 갱신, `on_complete: save_interview` → MCP 저장 등)
...
| `server_state` | `{ type: "text", content: "verified" }`    | MCP `verify_discord` 또는 `verify_server_state`가 서버 상태 확인 |
...
1. **블록 완료 시 (Day 1-7)**: specVersions가 업데이트되면 authenticated가 true인 경우 `save_spec_iteration` MCP 호출로 현재 버전을 서버에 동기화한다.
   - 새 버전을 추가한 경우: 해당 `version`의 canonical field 전체를 저장한다.
   - 기존 `version`의 `monetizationCheckpoint` 같은 필드를 Day 6에 보강한 경우: 같은 `version`을 다시 `save_spec_iteration` 호출해 서버 `spec_iterations`도 최신 checkpoint로 덮어쓴다.
...
## 15. Extended Rule Hand-off

아래 규칙은 standard Week 1 core path에서 항상 필요하지 않습니다.

- 위기점 분기
- archetype 기반 확장 분기
- NPC 취약성 모멘트 허용 규칙

이런 흐름이 필요한 block/context에서는 `references/extended/narrative-engine-extended.md`를 additive로 읽습니다.
