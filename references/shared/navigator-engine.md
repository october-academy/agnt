...
**모든 커맨드는 이 기본값을 사용** — v2 state에서 `/agnt:start` 없이 직접 커맨드 실행해도 안전.
...
`meta.schema_version == 2`이면 v2 스키마다. 아래 규칙을 적용한다:

1. `identity` 섹션 없으면 기본값 삽입:
...
| 3 | `!artifacts.competitors_analyzed` | `/agnt:compete` | "인터뷰에서 현재 대안을 들었어요. 경쟁 환경을 체계적으로 정리하세요." |
| 4 | `artifacts.spec_versions == 0` | `/agnt:spec` | "경쟁 분석 완료. 이제 SPEC으로 제품을 정리할 시점이에요." |
| 5 | `!artifacts.landing_deployed` | `/agnt:landing` | "SPEC이 있으니 랜딩페이지를 만들어 반응을 확인해보세요. MVP를 먼저 만들려면 `/agnt:build`." |
| 6 | `artifacts.channels_active < 2` | `/agnt:channel` | "랜딩이 있으니 사람들에게 보여줄 채널 {N}/2를 선정하세요." |
| 7 | `!artifacts.content_planned` | `/agnt:content` | "채널이 있어요. 거기에 뭘 올릴지 콘텐츠 전략을 세우세요." |
| 8 | `(signals.link_clicks > 0 \|\| (artifacts.channels_active >= 2 && artifacts.content_planned)) && !artifacts.offer_drafted` | `/agnt:offer` | "오퍼를 설계할 시점이에요. 클릭 데이터 또는 채널+콘텐츠 준비가 됐어요." |
| 9 | `artifacts.offer_drafted && !artifacts.launch_planned` | `/agnt:launch` | "오퍼가 준비됐어요. 론칭 계획을 세우세요." |
| 10 | `artifacts.launch_planned && artifacts.last_analyze_loop < artifacts.loops_completed + 1` | `/agnt:analyze` | "론칭 후 결과를 분석할 시점이에요. 성과를 판정하세요." |
| 11 | `artifacts.last_analyze_loop > artifacts.loops_completed` | `/agnt:retro` | "분석이 끝났어요. 회고하고 다음 루프를 계획하세요." |
| 12 | 위 모두 해당 없음 | `/agnt:status` | "모든 단계를 완료했어요! 현재 상태를 확인해보세요." |
...
| `/agnt:landing` | `artifacts.landing_deployed = true` | `submit_practice("wf-landing")` |
...
| `/agnt:offer` | `artifacts.offer_drafted = true`, `meta.last_action = "offer"`, `meta.total_actions++` | `submit_practice("wf-offer")` |
| `/agnt:launch` | `artifacts.launch_planned = true`, `meta.last_action = "launch"`, `meta.total_actions++` | `submit_practice("wf-launch")` |
| `/agnt:analyze` | `artifacts.last_analyze_loop = loops_completed + 1`, `meta.last_action = "analyze"`, `meta.total_actions++` | `get_landing_analytics`, `get_links`, `submit_practice("wf-analyze-1")` |
...
> **`/agnt:next` read-only 규칙 예외**: 기본적으로 next는 state를 읽기만 한다. 단, Identity Overlay CTA 표시 후 `sync.last_cta_nudge_at` 쓰기는 예외 허용 (기존 MCP 시그널 동기화 예외와 동일 패턴).
...
| `/agnt:landing` | `## Market` 내 `- 랜딩 URL:` | Replace | 덮어쓰기 |
...
모든 커맨드는 `state.json` 로드 후 아래 규칙을 적용한다:

- `meta.schema_version < 2` → 에러: "state.json이 v1(RPG) 스키마입니다. `/agnt:start`를 먼저 실행하세요."
- `meta.schema_version >= 2` (v2, v3 모두) → 정상 진행
- `meta.schema_version == 2` → v2→v3 마이그레이션 규칙 적용 후 진행 (identity/sync 기본값 삽입)

**적용 범위**: discover, interview, spec, compete, build, landing, channel, content, offer, launch, analyze, retro, status, next — 14개 커맨드 전체.
...
- Phase 2 (build + landing + channel×2 + content): 400 XP → 누적 1000 → Lv.7
- Phase 3 (offer + launch + analyze + retro): 310 XP → 누적 1310 → Lv.9
