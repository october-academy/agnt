...
- C: "이 커리큘럼은 수익 0에서 시작하는 사람을 위해 설계됐어. 계속할 수는 있지만, Day 1-7의 문제 정의, ICP 인터뷰 단계가 불필요하게 느껴질 수 있어."
- D: 추가 응답 없이 Q1으로 진행.

**Q1**: "지금 풀고 싶은 문제가 있어?"
- A) 있어 — 구체적으로 입력받음
- B) 아직 없어 — "괜찮아. 문제를 찾는 것부터 시작하면 돼."

Q1에서 B를 선택한 경우, 완료 출력 직전에 추가:

AskUserQuestion: "바로 `/agnt:discover`를 실행해볼까?"
- A) 바로 실행 — "/agnt:discover를 실행해." 안내
- B) 나중에 할게 — 표준 완료 출력 (Day 1/30 + /agnt:next 안내) 표시
