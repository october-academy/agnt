...
- `meta.schema_version < 2` → "먼저 `/agnt:start`로 업그레이드하세요." 출력 후 종료

`identity`와 `sync` 섹션이 없으면 기본값 적용 (v2→v3 마이그레이션 미적용 상태):

```
identity 섹션 없으면 → { mode: "guest", mcp: { connected: false, last_checked_at: null } }
identity.mode 없으면 → "guest"
identity.mcp.connected 없으면 → false
sync 섹션 없으면 → { pending_events: [], last_synced_at: null, last_inline_nudge_at: null, last_cta_nudge_at: null }
sync.pending_events 없으면 → []
```
