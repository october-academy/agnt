...
commands/              # /agnt:* 커맨드 (Claude Code가 실행하는 프롬프트)
  ...
  ├── audit.md         # /agnt:audit — Revenue Readiness Audit (45분 진단, Clarity Dimensions 정량 평가, Push×3, Anti-sycophancy → Track A/B/C 판정)
  ...
  ├── landing.md       # /agnt:landing — 랜딩 전략 가이드 (VOC 통합, 15+ 헤드라인 공식, 11섹션 구조, JTBD, Seven Sweeps, Anti-AI-slop)
  ...
  ├── tools.md         # /agnt:tools — 도구 비교 가이드 (사업자 상태 연동)
  ├── status.md        # /agnt:status — 현재 상태 대시보드
  ├── seo-audit.md     # /agnt:seo-audit — SEO+GEO 점검 (P0/P1/P2 계층, AI 봇 검증, E-E-A-T, 사이트별 맞춤)
  ├── biz-setup.md     # /agnt:biz-setup — 사업자 등록 판단 (의사결정 트리 + 비용 시뮬레이션)
  ├── analytics-setup.md # /agnt:analytics-setup — 분석 환경 세팅 (PostHog + 이벤트 추천)
  ├── launch-copy.md   # /agnt:launch-copy — 런칭 카피 엔진 (10채널 카피 + UTM 링크)
  ├── ad-creative.md   # /agnt:ad-creative — 광고 소재 Lab (3변형 + 가설 태그 + 예산)
  └── revenue.md       # /agnt:revenue — 수익 나침반 (시그널 기반 모델 추천)
...
references/            # 워크플로우 참조 콘텐츠
  ...
  ├── seo/             # SEO + GEO 점검
  │   ├── seo-checklist.md         # P0/P1/P2 계층 체크리스트 (42항목)
